"""
QuantDesk host — a console chatbot that uses Claude (via the Anthropic API)
as the coordinating LLM, and one or more MCP servers as its tools.
...
Functional requirements covered here (see PDF "Proyecto 1", section 3.1):
    ...
    2) Context maintained across a conversation (message history is kept
       and resent every turn).
...
from config import ANTHROPIC_MODEL, MAX_TOKENS, MAX_TOOL_ITERATIONS, SERVERS, ensure_git_initialized
...
async def run_chat():
    ...
    async with MCPClientManager(SERVERS, jsonrpc_logger) as manager:
        ...
        print("QuantDesk ready. Type your question ('exit' to quit).\n")
